...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 组内父子树：depth = 到根的层数，order = 同层内访问顺序（前序遍历）。
     *  树状排版：父在左列、子向右缩进一列、同层垂直排开 → 分叉一目了然且不重叠。 */
    function groupTree(members) {
      const ids = new Set(members.map((m) => m.id));
      const children = new Map();
      const roots = [];
      for (const m of members) {
        const pid = m.parentId !== null && ids.has(m.parentId) ? m.parentId : null;
        if (pid === null) roots.push(m);
        else {
          const arr = children.get(pid);
          if (arr === void 0) children.set(pid, [m]);
          else arr.push(m);
        }
      }
      const info = new Map();
      ...
      const walk = (node, depth) => {
        const order = perDepth.get(depth) ?? 0;
        perDepth.set(depth, order + 1);
        info.set(node.id, { depth, order });
        if (depth > maxDepth) maxDepth = depth;
        const kids = children.get(node.id);
        if (kids !== void 0) for (const k of kids) walk(k, depth + 1);
      };
      for (const r of roots) walk(r, 0);
      for (const m of members) if (!info.has(m.id)) walk(m, 0);
      let maxCount = 0;
      for (const c of perDepth.values()) if (c > maxCount) maxCount = c;
      return { children, roots, info, maxDepth, maxCount };
    }
    ...
    function buildBoard(forest, saved) {
      ...
      // Saved frames keep the user's x/y/w, but their HEIGHT grows to fit
      // every member — a new session must never overflow a pinned frame.
      // Sizing follows the tree layout (depth = width, column height = height).
      const frames = {};
      let cursorX = WORLD_PAD;
      let cursorY = WORLD_PAD;
      for (const g of groups) {
        const savedFrame = savedFrames[g.key];
        const t = groupTree(g.members);
        const autoW = FRAME_PAD * 2 + (t.maxDepth + 1) * NODE_W + t.maxDepth * CARD_GAP_X;
        const autoH = FRAME_TITLE_H + FRAME_PAD * 2 + t.maxCount * NODE_H + (t.maxCount - 1) * CARD_GAP_Y;
        if (savedFrame !== void 0) {
          frames[g.key] = { x: savedFrame.x, y: savedFrame.y, w: savedFrame.w, h: Math.max(savedFrame.h, autoH) };
          continue;
        }
        frames[g.key] = { x: cursorX, y: cursorY, w: autoW, h: autoH };
        cursorX += autoW + FRAME_GAP_X;
      }

      // Card positions: saved wins; else TREE layout (parent left, children
      // indented one column right, siblings stacked vertically in their column).
      ...
      for (const g of groups) {
        ...
        for (const n of g.members) {
          ...
          const info = t.info.get(n.id) ?? { depth: 0, order: 0 };
          let order = info.order;
          while (taken.has(key(info.depth, order))) order++;
          taken.add(key(info.depth, order));
          positions.set(n.id, {
            x: f.x + FRAME_PAD + info.depth * (NODE_W + CARD_GAP_X),
            y: f.y + FRAME_TITLE_H + FRAME_PAD + order * (NODE_H + CARD_GAP_Y),
          });
        }
      }

      // Collision + edge avoidance, one shared strategy:
      //   1) find the nearest FREE grid slot INSIDE the frame (no blind
      //      push-right/down that would overflow the workspace box),
      //   2) if the frame has no free slot, extend the frame itself.
      const groupOfNode = new Map();
      for (const g of groups) for (const m of g.members) groupOfNode.set(m.id, g);
      const overlapRect = (a, b) => a.x < b.x + NODE_W && a.x + NODE_W > b.x && a.y < b.y + NODE_H && a.y + NODE_H > b.y;
      const insideFrame = (f, cand) =>
        cand.x + NODE_W <= f.x + f.w - FRAME_PAD && cand.y + NODE_H <= f.y + f.h - FRAME_PAD;
      /** Nearest free grid slot inside `f`, near the node's current tree slot. */
      function freeSpotFor(g, f, t, nodeId, allPositions, excludeId) {
        const info = t.info.get(nodeId) ?? { depth: 0, order: 0 };
        const sd = info.depth, so = info.order;
        let best = null, bestDist = Infinity;
        for (let d = 0; d <= t.maxDepth + 4; d++) {
          for (let o = 0; o <= t.maxCount + 4; o++) {
            const cand = {
              x: f.x + FRAME_PAD + d * (NODE_W + CARD_GAP_X),
              y: f.y + FRAME_TITLE_H + FRAME_PAD + o * (NODE_H + CARD_GAP_Y),
            };
            if (!insideFrame(f, cand)) continue;
            const clash = [...allPositions.entries()].some(([id, r]) => id !== excludeId && overlapRect(cand, r));
            if (clash) continue;
            const dist = Math.abs(d - sd) * 3 + Math.abs(o - so);
      ...
      // --- collision evacuation (auto cards never overlap, never leave frame) ---
      const settled = new Map();
      for (const [id, pos] of positions) {
        if (savedCards[id] !== void 0) { settled.set(id, pos); continue; }
        const g = groupOfNode.get(id);
        const f = g !== void 0 ? frames[g.key] : void 0;
        const t = g !== void 0 ? groupTree(g.members) : null;
        let cand = pos;
        if (f !== void 0 && [...settled.entries()].some(([oid, r]) => overlapRect(cand, r))) {
          const spot = freeSpotFor(g, f, t, id, settled, id);
          if (spot !== null) cand = spot;
          else {
            const d = t.maxDepth + 1, o = t.maxCount;
            cand = {
              x: f.x + FRAME_PAD + d * (NODE_W + CARD_GAP_X),
              y: f.y + FRAME_TITLE_H + FRAME_PAD + o * (NODE_H + CARD_GAP_Y),
            };
            let guard = 0;
            while (guard < 30 && [...settled.entries()].some(([oid, r]) => overlapRect(cand, r))) {
              cand = { x: cand.x + NODE_W + CARD_GAP_X, y: cand.y };
              guard++;
            }
            frames[g.key] = {
              x: f.x, y: f.y, w: Math.max(f.w, cand.x + NODE_W + FRAME_PAD - f.x),
              h: Math.max(f.h, cand.y + NODE_H + FRAME_PAD - f.y),
            };
          }
        }
      ...
      // --- edge avoidance: sample the real bezier, move hit cards to a free
      //     slot inside the frame (or grow the frame), saved cards included ---
      const byId2 = new Map();
      for (const g of groups) for (const m of g.members) byId2.set(m.id, m);
      const rectHit = (s, q) => s.x >= q.x && s.x <= q.x + NODE_W && s.y >= q.y && s.y <= q.y + NODE_H;
      for (let iter = 0; iter < 10; iter++) {
        let moved = false;
        const cur = new Map(positions);
        for (const m of byId2.values()) {
          if (m.parentId === null) continue;
          const p = cur.get(m.parentId), c = cur.get(m.id);
          if (p === void 0 || c === void 0) continue;
          const x1 = p.x + NODE_W / 2, y1 = p.y + NODE_H / 2;
          const x2 = c.x + NODE_W / 2, y2 = c.y + NODE_H / 2;
          const my = (y1 + y2) / 2;
          const samples = [0.12, 0.25, 0.4, 0.5, 0.6, 0.75, 0.88].map((t) => {
            const u = 1 - t;
            return {
              x: u * u * x1 + 2 * u * t * x1 + t * t * x2,
              y: u * u * y1 + 2 * u * t * my + t * t * y2,
            };
          });
          for (const m2 of byId2.values()) {
            if (m2.id === m.id || m2.id === m.parentId) continue;
            const q = cur.get(m2.id);
            if (q === void 0) continue;
            if (!samples.some((s) => rectHit(s, q))) continue;
            const g2 = groupOfNode.get(m2.id);
            const f2 = g2 !== void 0 ? frames[g2.key] : void 0;
            const t2 = g2 !== void 0 ? groupTree(g2.members) : null;
            if (f2 === void 0 || t2 === null) continue;
            // find a free slot against the LIVE positions map so two cards
            // moved in the same pass never land on the same spot
            const spot = freeSpotFor(g2, f2, t2, m2.id, positions, m2.id);
            if (spot !== null) { positions.set(m2.id, spot); moved = true; }
            else {
              const d = t2.maxDepth + 1, o = t2.maxCount;
              let cand = {
                x: f2.x + FRAME_PAD + d * (NODE_W + CARD_GAP_X),
                y: f2.y + FRAME_TITLE_H + FRAME_PAD + o * (NODE_H + CARD_GAP_Y),
              };
              // grow-slot must not collide either; nudge right/down until clear
              let guard = 0;
              while (guard < 30 && [...positions.entries()].some(([id, r]) => id !== m2.id && overlapRect(cand, r))) {
                cand = { x: cand.x + NODE_W + CARD_GAP_X, y: cand.y };
                guard++;
              }
              positions.set(m2.id, cand);
              frames[g2.key] = {
                x: f2.x, y: f2.y, w: Math.max(f2.w, cand.x + NODE_W + FRAME_PAD - f2.x),
                h: Math.max(f2.h, cand.y + NODE_H + FRAME_PAD - f2.y),
              };
              moved = true;
            }
          }
...
.cm-edge{fill:none;stroke:${COLORS.accent};stroke-width:2.5;opacity:1}
.cm-edge[data-dim="1"]{opacity:.2}
...
`;
    ...
    function MapNode({ p, selected, current, dim, digest, onSelect, onOpen, onRefreshDigest, onDragStart, overlap, dragging, onContextMenu }) {
      ...
      return React.createElement(
        ...
        // card body — 边框继承工作区主题色；未选中浅(35%)，选中/当前实(100%)
        React.createElement("rect", {
          className: "box", width: NODE_W, height: NODE_H, rx: 12,
          fill: tag.fill,
          style: {
            stroke: tag.border,
            strokeOpacity: selected || current ? 1 : 0.45,
            strokeWidth: selected || current ? 2 : 1,
    ...
    function MapCanvas(props) {
      ...
      // node → 所属工作区主题色（连线颜色继承工作区）
      const groupTag = new Map();
      for (const g of model.groups) {
        const tag = colorTagFor(g.members[0] ?? { cwdBase: g.title, preset: "", origin: "" });
        for (const m of g.members) groupTag.set(m.id, tag);
      }
      const edges = [];
      for (const n of byId.values()) {
        ...
        const my = (y1 + y2) / 2;
        const edgeTag = groupTag.get(n.id) ?? groupTag.get(parent.id);
        edges.push(
          React.createElement("path", {
            ...
            style: { fill: "none", stroke: edgeTag !== void 0 ? edgeTag.border : COLORS.accent, strokeWidth: 2.5 },
          }),
        );
      }
